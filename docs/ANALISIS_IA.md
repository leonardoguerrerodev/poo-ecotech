...
| 39 | El diagrama `modelo_u3.drawio` (hoy `EcoTech_UML_VIGENTE.drawio`) | Las filas nuevas de `Usuario` se agregaron al final del XML con su coordenada `y` | Error | **Modificar** | La caja usa `stackLayout`, que ordena las filas por su posición en el XML e ignora la `y`: los tres métodos nuevos habrían aparecido debajo de `validarClave`. Se reubicaron en el XML, después de `rol` y de `tienePermiso` |
...
| 45 | Conteo de los miembros del diagrama para la matriz de la rúbrica | La IA sumó **68** a mano y lo escribió en la matriz; un script que extrae las celdas del XML dio **66** | Error | **Descartar** el conteo manual | Toda cifra de la matriz sale ahora de un script. El error venía de atribuir 8 filas a `Proyecto` y a `RegistroTiempo`, que tienen 7 |
